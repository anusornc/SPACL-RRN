...
use crate::{ReasoningStrategy, OntologyFeatures, ReasoningTask};
...
impl EvolutionaryOptimizer {
    ...
    fn create_random_strategy(&mut self) -> EvolutionaryStrategy {
        ...
        selection_thresholds.insert("tableaux_threshold".to_string(), self.rng.gen_range(0.3..0.9));
        selection_thresholds.insert("saturation_threshold".to_string(), self.rng.gen_range(0.2..0.8));
        selection_thresholds.insert("transformation_threshold".to_string(), self.rng.gen_range(0.1..0.7));
        
        let mut timeouts = HashMap::new();
        timeouts.insert(ReasoningStrategy::Tableaux, self.rng.gen_range(5000..60000));
        timeouts.insert(ReasoningStrategy::Saturation, self.rng.gen_range(1000..30000));
        timeouts.insert(ReasoningStrategy::Transformation, self.rng.gen_range(2000..45000));
        ...
        while offspring.len() + current_size < self.population_size {
            if self.rng.gen::<f64>() < self.crossover_rate {
                ...
                if parent1_idx != parent2_idx {
                    let child = self.create_offspring(
    ...
    fn create_offspring(
        ...
        for (key, value1) in &parent1.parameters.selection_thresholds {
            if let Some(value2) = parent2.parameters.selection_thresholds.get(key) {
                let alpha = self.rng.gen::<f64>();
                let new_value = alpha * value1 + (1.0 - alpha) * value2;
                child_params.selection_thresholds.insert(key.clone(), new_value);
            }
        }
        ...
        for (strategy, timeout1) in &parent1.parameters.timeouts {
            if let Some(timeout2) = parent2.parameters.timeouts.get(strategy) {
                let alpha = self.rng.gen::<f64>();
                let new_timeout = (alpha * (*timeout1 as f64) + (1.0 - alpha) * (*timeout2 as f64)) as u64;
        ...
        let alpha = self.rng.gen::<f64>();
        child_params.cache_config.primary_cache_size = 
            ((alpha * parent1.parameters.cache_config.primary_cache_size as f64) +
             ((1.0 - alpha) * parent2.parameters.cache_config.primary_cache_size as f64)) as usize;
        
        child_params.cache_config.secondary_cache_size = 
            ((alpha * parent1.parameters.cache_config.secondary_cache_size as f64) +
             ((1.0 - alpha) * parent2.parameters.cache_config.secondary_cache_size as f64)) as usize;
    ...
    fn mutate_strategy(&mut self, strategy: &mut EvolutionaryStrategy) {
        ...
        if self.rng.gen::<f64>() < 0.3 {
            ...
            strategy.parameters.cache_config.primary_cache_size = 
                (strategy.parameters.cache_config.primary_cache_size as i32 + change).max(50) as usize;
        }
    }
