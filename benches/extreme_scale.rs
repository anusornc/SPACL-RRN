...
fn bench_spacl(ontology: &Ontology) -> bool {
    ...
    let mut reasoner = SpeculativeTableauxReasoner::with_config(
        ontology.clone(),
        config,
    );
    let result = reasoner.is_consistent().unwrap_or(false);
    let stats = reasoner.get_stats();
    println!("\nStats: {} branches, {} pruned", stats.branches_created, stats.branches_pruned);
    result
}
...
fn extreme_scale_benchmark(c: &mut Criterion) {
    ...
    for size in [10000, 50000, 100000] {
        ...
        group.bench_with_input(
            ...
            &ontology,
            |b, o| b.iter(|| bench_sequential(black_box(o))),
        );
        
        // SPACL
        group.bench_with_input(
            BenchmarkId::new("spacl", size),
            &ontology,
            |b, o| b.iter(|| bench_spacl(black_box(o))),
        );
    }
