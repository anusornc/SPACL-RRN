...
fn main() {
    ...
    let seq = benchmark("Sequential", || {
        SimpleReasoner::new(ontology.clone()).is_consistent().unwrap()
    }, 10000);
    let adaptive = benchmark("Adaptive", || {
    ...
    let adaptive_low = benchmark("Adaptive (low thresh)", || {
        let mut config = SpeculativeConfig::default();
        config.parallel_threshold = 50;  // Will use sequential
        SpeculativeTableauxReasoner::with_config(ontology.clone(), config)
            .is_consistent().unwrap()
    }, 5000);
    let adaptive_high = benchmark("Adaptive (high thresh)", || {
        let mut config = SpeculativeConfig::default();
    ...
    println!("  Sequential speedup vs parallel: {:.1}x\n", adaptive_high / seq);
