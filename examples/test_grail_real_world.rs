...
fn main() {
    ...
    for (name, path) in ontologies {
        ...
        let file_size_mb = std::fs::metadata(path).map(|m| m.len() as f64 / 1_048_576.0).unwrap_or(0.0);
        
        println!("  ✓ Loaded: {} classes, {} axioms ({:.1} MB) in {:?}", 
        ...
        println!("  Can use hierarchical: {} (checked in {:?})", can_handle, check_time);
        ...
        println!("  Simple subclass axioms: {}/{} ({:.1}%)", 
                 simple_count, simple_count + complex_count,
