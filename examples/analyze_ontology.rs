...
use owl2_reasoner::{Ontology, OntologyCharacteristics, ReasoningStrategy, ParserFactory};
use std::env;
use std::path::Path;
...
fn main() {
    ...
    let ontology_path = args.get(1).map(|s| s.as_str()).unwrap_or(
        "benchmarks/ontologies/other/go-basic.owl"
    );
    ...
    println!("  • Complex Expressions: {}", chars.complex_expression_count);
    println!("  • Disjointness Axioms: {}", chars.disjointness_axiom_count);
    ...
    println!("  • Tree-like:        {}", if chars.is_tree_like { "✓ Yes" } else { "✗ No" });
    println!();
    ...
    println!("  • Fast Path Eligible: {}", if chars.can_use_fast_path() { "✓ Yes" } else { "✗ No" });
    println!("  • Is Small:         {}", if chars.is_small() { "✓ Yes" } else { "✗ No" });
    println!("  • Strategy:         {:?}", chars.recommended_strategy);
    println!("  • Est. Time:        ~{}ms", chars.estimated_classification_time_ms());
    println!();
