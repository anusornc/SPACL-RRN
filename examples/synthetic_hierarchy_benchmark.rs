...
fn main() {
    ...
    for size in sizes {
        ...
        println!("  Generated: {} classes, {} axioms", class_count, axiom_count);
        ...
        println!("  Running hierarchical ({} iterations)...", hierarchical_iters);
        ...
        if size <= 5000 {
            ...
            let avg_simple = simple_times.iter().sum::<std::time::Duration>() / simple_times.len() as u32;
            ...
            println!("  Skipping simple reasoner (would be too slow for {} classes)", size);
...
fn generate_tree_ontology(n: usize) -> Ontology {
    ...
    for i in 0..n {
        ...
        ontology.add_axiom(Axiom::SubClassOf(Box::new(axiom))).unwrap();
