...
use std::io::Write;
use owl2_reasoner::{
    Ontology, OwlReasoner, ParserFactory,
    SimpleReasoner, HierarchicalClassificationEngine,
};
...
fn main() {
    ...
    log(&log_file, "=== Complete Real-World Ontology Benchmark ===\n");
...
fn benchmark_ontology(name: &str, path: &str, log_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    log(log_file, &format!("  Can use hierarchical: {}\n", can_hierarchical));
    ...
    println!("  Simple subclass axioms: {}/{} ({:.1}%)", 
             simple_count, total_subclass, simple_ratio);
    log(log_file, &format!("  Simple subclass axioms: {}/{} ({:.1}%)\n", 
                          simple_count, total_subclass, simple_ratio));
    ...
    let hierarchical_time = if can_hierarchical {
        ...
        for i in 0..iterations {
            ...
            if i == 0 {
                ...
                log(log_file, &format!("  First hierarchical run: {:?}\n", elapsed));
            }
        }
        
        // Calculate average (exclude first run as warm-up)
        let avg_time: std::time::Duration = times.iter().skip(1).sum::<std::time::Duration>() 
            / (times.len() - 1).max(1) as u32;
        
        println!("  Average hierarchical time: {:?}", avg_time);
        log(log_file, &format!("  Average hierarchical time: {:?}\n", avg_time));
    ...
    log(log_file, &format!("  Average simple time: {:?}\n", avg_simple));
    ...
    if let Some(hier_time) = hierarchical_time {
        ...
        log(log_file, "\n  RESULTS:\n");
        log(log_file, &format!("  - Simple: {:?}\n", avg_simple));
        log(log_file, "  - Strategy: Simple (hierarchical not applicable)\n");
    }
