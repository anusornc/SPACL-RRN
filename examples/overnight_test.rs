...
fn test_ontology(name: &str, path: &str) -> Result<BenchmarkResult, String> {
    ...
    let ontology: Ontology = parser.parse_str(&content)
    ...
    match seq_result {
        Ok(consistent) => {
            ...
            log(&format!("  Result: {}", if consistent { "CONSISTENT" } else { "INCONSISTENT" }));
        }
        Err(e) => {
...
fn print_summary(results: &[BenchmarkResult]) {
    ...
    println!("{:<12} {:>8} {:>10} {:>12} {:>12} {:>10}",
        "Ontology", "Size(MB)", "Classes", "Seq(ms)", "SPACL(ms)", "Speedup");
    println!("{}", "─".repeat(80));
    
    for r in results {
        println!("{:<12} {:>8.1} {:>10} {:>12} {:>12} {:>9.2}x",
...
fn main() {
    ...
    log(&format!("Finished at: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S")));
}
