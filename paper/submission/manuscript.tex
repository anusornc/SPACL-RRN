...
\begin{keyword}
\dlfrag \sep Tableaux Reasoning \sep Parallel Algorithms \sep Nogood Learning \sep Description Logics \sep Semantic Web
\end{keyword}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\linewidth]{scalability.pdf}\hfill
\includegraphics[width=0.48\linewidth]{speedup.pdf}

\vspace{0.6em}
\includegraphics[width=0.72\linewidth]{throughput.pdf}
\caption{Synthetic scalability visual summary corresponding to Table~\ref{tab:scalability}. Top row: runtime trend (left) and speedup over the sequential baseline (right). Bottom row: throughput comparison.}
\label{fig:scalability-panels}
\end{figure}

The crossover appears around 1,000 classes, and Figure~\ref{fig:scalability-panels} shows the same transition visually: below that point, speculative overhead largely cancels any gain; above it, the branch structure is large enough for parallelism to amortize coordination cost. Throughput here denotes synthetic expansion operations per second in the microbenchmark harness. The right panel compares both sequential and SPACL throughput, while the throughput column in Table~\ref{tab:scalability} reports the SPACL throughput values used for compact tabular comparison. This is consistent with the conservative adaptive-gating design and explains why the main real-world evidence is separated from the synthetic scalability signal.
