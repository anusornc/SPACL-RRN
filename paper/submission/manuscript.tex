...
%% ============================================
\begin{abstract}
We present SPACL (Speculative Parallel Tableaux with Adaptive Conflict Learning), a novel ALC/SHOIQ reasoner that achieves significant performance improvements through speculative parallelism combined with conflict-driven learning. To the best of our knowledge, SPACL is among the first open implementations to combine work-stealing parallelism with nogood learning for this setting, addressing the challenge of exponential search spaces in tableau-based reasoning.

Our key contributions include: (1) a speculative parallel tableaux algorithm that explores branches concurrently using work-stealing; (2) an adaptive threshold mechanism that automatically selects between sequential and parallel processing based on problem complexity, achieving 18--73$\times$ speedup over naive parallelization for small ontologies; (3) thread-local nogood caching that reduces synchronization overhead by 80\%; (4) a global thread pool optimization reducing overhead by 2.1$\times$; and (5) a production-quality Rust implementation with comprehensive parser support for RDF/XML, OWL2 Functional, Manchester, and JSON-LD syntaxes. SPACL demonstrates $5\times$ speedup at 10,000 classes while maintaining $<2\times$ overhead for small ontologies.

Benchmarks demonstrate \textbf{40--707$\times$ wall-clock speedup} on small disjunctive ontologies ($<$1,000 classes) vs HermiT; however, this includes significant JVM startup overhead ($\sim$3--4s). For pure reasoning time (excluding JVM startup), algorithmic speedup is 15--30$\times$. In our latest large-suite head-to-head run (\texttt{head2head\_round4\_large\_full\_20260219\_rerun}), SPACL records the best wall-clock result on all five tested ontologies (chebi, doid, go-basic, pato, uberon) under an identical 900s timeout protocol. This improvement is driven by a full-fidelity structural RDF/XML parser and optimized materialization path: stage-level A/B runs show parse-only reductions of 96.96--97.98\% on doid/go-basic/uberon relative to the previous structural baseline.
\end{abstract}
...
%% ============================================
\begin{highlights}
\item To our knowledge, one of the first open ALC/SHOIQ implementations combining speculative parallelism with nogood learning
...
\item Structural RDF/XML parser + full-fidelity materialization reduces parse-only time by 96.96--97.98\% on large ontologies
...
\textbf{To the best of our knowledge, SPACL is among the first open systems to integrate work-stealing parallelism with nogood learning for ALC/SHOIQ tableau reasoning}, combining the throughput benefits of speculative parallelism with the search-space reduction of conflict-driven learning.
...
\begin{enumerate}
    ...
    \item \textbf{Reproducibility}: Primary artifact set for this paper revision is \texttt{benchmarks/competitors/results/history/head2head\_round4\_large\_full\_20260219\_rerun} (CSV, markdown/LaTeX tables, and raw per-reasoner JSON outputs). Parser-stage A/B artifacts are \texttt{stages\_doid\_20260218\_183604} vs \texttt{stages\_doid\_20260219\_130432}, \texttt{stages\_go\_basic\_20260218\_185126} vs \texttt{stages\_go\_basic\_20260219\_130710}, and \texttt{stages\_uberon\_20260218\_185933} vs \texttt{stages\_uberon\_20260219\_130942}.
\end{enumerate}
...
\textbf{Key Findings}:
\begin{itemize}
    \item SPACL is fastest on 5/5 ontologies in this run set (\texttt{head2head\_round4\_large\_full\_20260219\_rerun})
...
We presented SPACL, an open-source ALC/SHOIQ reasoner combining speculative parallelism with conflict-driven learning. On small disjunctive ontologies, SPACL shows large wall-clock gains versus HermiT (up to 707$\times$), with algorithmic gains estimated at 15--30$\times$ after controlling for JVM startup effects. For large biomedical RDF/XML ontologies, the revised structural parser/materialization pipeline substantially changes end-to-end behavior: in the reproducible run set \texttt{head2head\_round4\_large\_full\_20260219\_rerun}, SPACL achieves the best wall-clock result on all five tested ontologies while completing all runs within timeout.
...
SPACL is available at \url{https://github.com/anusornc/tableauxx} under the MIT license. All benchmarks and test data are included for reproducibility.

\subsection{Submission Reproducibility Checklist}
\label{sec:repro-checklist}

\begin{enumerate}
    \item \textbf{Source and build}: Use the repository state corresponding to this manuscript revision and build with Rust release profile (\texttt{-C opt-level=3 -C lto=fat}).
    \item \textbf{Hardware disclosure}: Record CPU model, core/thread count, RAM size, OS version, and container runtime version.
    \item \textbf{Primary head-to-head run}: Reproduce the large-suite benchmark using run ID \texttt{head2head\_round4\_large\_full\_20260219\_rerun}; compare against \texttt{benchmarks/competitors/results/history/head2head\_round4\_large\_full\_20260219\_rerun/results.csv}.
    \item \textbf{Stage-level parser A/B}: Reproduce stage pairs for DOID/GO\_Basic/UBERON:
    \begin{itemize}
        \item old: \texttt{stages\_doid\_20260218\_183604}, \texttt{stages\_go\_basic\_20260218\_185126}, \texttt{stages\_uberon\_20260218\_185933}
        \item new: \texttt{stages\_doid\_20260219\_130432}, \texttt{stages\_go\_basic\_20260219\_130710}, \texttt{stages\_uberon\_20260219\_130942}
    \end{itemize}
    \item \textbf{Parser configuration}: For structural parser results, set \texttt{OWL2\_REASONER\_STRUCTURAL\_XML\_PARSER=1}. For full text-path forcing, set \texttt{OWL2\_REASONER\_FORCE\_TEXT=1}, \texttt{OWL2\_REASONER\_BIN\_ONLY=0}, and \texttt{OWL2\_REASONER\_AUTO\_CACHE=0}.
    \item \textbf{Timeout policy}: Use \texttt{TIMEOUT\_SECONDS=900} for large-suite comparisons; report \texttt{success}, \texttt{timeout}, \texttt{failed}, and \texttt{not\_available} exactly as emitted by the harness.
    \item \textbf{Metric policy}: Treat container wall time as primary cross-engine metric; keep internal reported time as secondary diagnostics only.
    \item \textbf{Artifacts to archive}: Preserve \texttt{results.csv}, per-reasoner JSON outputs, \texttt{paper\_table.tex}, and \texttt{benchmark\_report.md} for each run.
    \item \textbf{Paper build}: Compile this manuscript with \texttt{paper/submission/compile.sh} and verify successful generation of \texttt{manuscript.pdf}.
    \item \textbf{Claim scope}: Restrict comparative claims to this disclosed run set and configuration; do not generalize to unmatched profiles or undisclosed environments.
\end{enumerate}
