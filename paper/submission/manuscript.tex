...
\hypersetup{
    ...
    urlcolor=cyan,
    citecolor=blue,
    pdftitle={SPACL: Speculative Tableaux and Structural RDF/XML Acceleration for OWL Ontology Reasoning},
...
\title{SPACL: Speculative Tableaux and Structural RDF/XML Acceleration for OWL Ontology Reasoning}
...
In the Standard Workload Suite (7 ontologies; 3 clean runs), per-ontology medians among successful comparable runs place SPACL lowest on 4/7 ontologies; ELK is lowest on the two deepest hierarchy files, and Pellet is lowest on \texttt{univ-bench.owl}. In the primary Large Head-to-Head benchmark (doid/go-basic/uberon; 3 clean runs), SPACL has the lowest median wall-clock time on all three tested ontologies among successful comparable runs under a common 900s timeout protocol. In repeated OWL2Bench core-profile validation (DL/EL/QL/RL), SPACL completes 12/12 runs and records the lowest median wall-clock time on 4/4 ontologies in this harness.
...
Parallel mode is enabled only when both a branch-count estimate and a lightweight cost score exceed conservative thresholds. The branch estimate is
\begin{align}
...
\node[box, below=of sched, xshift=-16mm] (w1) {Worker 1};
\node[box, below=of sched] (w2) {Worker 2};
\node[box, below=of sched, xshift=16mm] (wn) {Worker N};
\node[box, below=of gate, yshift=-18mm] (nogood) {Nogood Store\\+ Local Caches};
...
The implemented branch estimate is
\begin{equation}
...
To isolate parser/materialization effects, we performed controlled stage-level A/B runs before and after the structural RDF/XML pipeline changes. Table~\ref{tab:realworld} reports the primary biomedical pairs together with a supplementary ChEBI stress case under the revised cold-text contract.
...
This suite is best read as a deployment sanity check rather than as the strongest evidence for reasoning quality, because startup and parser costs are still visible at this scale. Within that scope, SPACL is lowest on 4/7 ontologies, ELK is lowest on the two deepest hierarchy files, and Pellet is lowest on \texttt{univ-bench.owl}. All systems except JFact complete every Standard-suite case under the 300s policy.
...
Among successful comparable runs, SPACL has the lowest median wall-clock time on all three ontologies in the primary large suite. SPACL, HermiT, and ELK complete every run under the 900s policy; Openllet and Pellet time out on UBERON, and JFact fails on UBERON in this harness.

\subsubsection{Failure-Aware Suite Summary}
\label{sec:cew}

Success-only per-ontology medians do not capture timeout penalties directly. We therefore also report a capped expected wall-time (CEW) summary over the primary comparison set, where timeout or failure is replaced by the suite timeout before averaging.
...
A supplementary matched three-run panel on \texttt{pato.owl} follows the same qualitative direction as the primary large suite. SPACL is the lowest successful median (3238 ms), ELK and HermiT remain successful but slower (6664 ms and 12362 ms), Pellet succeeds but is much slower (745159 ms), and Openllet/JFact time out on all three runs. We therefore use PATO as complementary biomedical evidence rather than as a replacement for the primary three-ontology panel. Aggregate summary: \path{results/history/pato_matched_aggregate_20260301/summary.csv}.
...
SPACL combines speculative work stealing, verified nogood reuse, and a structural RDF/XML pipeline in one open implementation for the evaluated ALC/SHOIQ handling path. The empirical picture is mixed but coherent, and the paper should be read that way. On synthetic disjunctive workloads, the adaptive gate avoids pathological always-parallel overhead and permits net speedup once branch structure becomes large enough. On the main real-world biomedical panel (DOID, GO-Basic, UBERON), however, the dominant gain in this revision comes from parser/materialization acceleration: stage-level A/B runs show about 97\% parse-only reduction, and SPACL records the lowest median end-to-end wall-clock time on all three ontologies among successful comparable runs in the primary large-suite campaign. The OWL2Bench core-profile suite shows the same overall direction, with SPACL completing all 12 runs and yielding the lowest median wall-clock time on the four tested profiles in this harness.

These conclusions remain bounded by the current prototype and protocol. The main comparisons are single-machine, repeated-run wall-clock results under a containerized CLI harness; they do not imply identical logic-profile coverage across all competitors, nor do they replace matched kernel-level reasoning measurements. PATO, ChEBI, and Konclude are therefore reported as supplementary evidence rather than folded into the primary comparison basis. Future work should extend coverage to broader inconsistent real-world corpora, deeper ABox evaluation, and cleaner deployment-neutral comparator campaigns. SPACL is available at \url{https://github.com/anusornc/SPACL}; reproduction details are summarized in \path{paper/submission/README.txt}.
