...
\end{align}
This is intentionally conservative. SPACL does not treat every contradictory trace as globally reusable; it only admits verified branch-level cores that remain contradiction-preserving under the current ontology and constraint context.
...
We model end-to-end parser load time as
\begin{equation}
T_{\text{total}} = T_{\text{ingest}} + T_{\text{materialize}} + T_{\text{io}}.
\label{eq:parser-total}
\end{equation}
For a cache layer $k \in \{\text{subj}, \text{pred}, \text{obj}\}$, the hit rate and expected lookup time are
\begin{equation}
...
To relate parser acceleration to overall wall-clock improvement, we use Amdahl's law:
\begin{equation}
...
These equations are explanatory bookkeeping relations for interpreting the measured stage shifts in Section~\ref{sec:realworld}; they are not presented as a fitted predictive ingest model.
