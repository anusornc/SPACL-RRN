...
Comprehensive benchmarks on synthetic hierarchies (100--10,000 classes) show SPACL achieves 26.2 million operations per second and 4.88$\times$ speedup at scale. Real-world evaluation on BioPortal ontologies (PATO, DOID, UBERON, GO) demonstrates 0.4--0.5$\times$ speedup, revealing that SPACL's parallelization benefits are realized on ontologies with disjunctive structures rather than pure taxonomic hierarchies. SPACL is the first open-source OWL2 DL reasoner to provide both speculative parallelism and conflict-driven learning with adaptive threshold selection.
\end{abstract}
...
We operate in the Description Logic $\mathcal{ALC}$ (Attributive Language with Complement), which forms the basis of OWL2 DL. The syntax is defined as follows:
...
We presented SPACL, the first open-source OWL2 DL reasoner combining speculative parallelism with conflict-driven learning. On synthetic hierarchies with controlled disjunctions, SPACL achieves $5\times$ speedup at 10,000 classes. Real-world ontology evaluation on BioPortal ontologies (PATO, DOID, UBERON, GO) shows 0.4--0.5$\times$ speedup, indicating SPACL is optimized for disjunctive reasoning rather than taxonomic hierarchies. The adaptive threshold mechanism correctly identifies when sequential processing is preferable, preventing overhead on unsuitable ontologies.
...
\begin{enumerate}
    \item \textbf{Real-World Performance Gap}: While SPACL achieves 4.88$\times$ speedup on synthetic hierarchies with controlled disjunctions, real-world ontologies (PATO, DOID, UBERON, GO) show 0.4--0.5$\times$ speedup (2--2.5$\times$ slower). This is because biomedical ontologies are primarily taxonomic hierarchies with sparse disjunctions ($<$0.1\% of axioms), limiting parallelization opportunities. SPACL's benefits are realized on ontologies with complex disjunctive structures.
