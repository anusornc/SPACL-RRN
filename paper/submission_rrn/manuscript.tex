...
This manuscript track focuses on protocol design and reproducibility boundaries for the hybrid study: identical ontology suites, timeout policy, and hardware across baseline and hybrid modes; repeated-run reporting with median/P95 wall time; and ablations that isolate guidance impact from parser and ingest effects. In the current branch-active synthetic policy study, a pairwise-trained hybrid model improves repeat-matched outcomes over the heuristic policy in both pinned-CPU confirmation passes (9/10 wins per pass on mixed-operand workloads), while variance remains high and claims are therefore bounded to policy-isolation workloads.
\end{abstract}
...
For hybrid guidance, we position RRN as a neural-symbolic policy candidate rather than as an entailment replacement. Recursive Reasoning Networks have been studied for ontology reasoning in neural settings \cite{hohenecker2020ontology}; in this companion track we borrow only the guidance intuition (learned relational prioritization) while keeping symbolic tableaux decisions authoritative. We also include a non-neural tree-ensemble ranking comparator (gradient boosting over decision stumps \cite{friedman2001greedy}) to reduce model-choice bias in policy-level conclusions.
...
In repeat-matched comparisons on this locked pass, linear-v3 and heuristic split wins (5/10 each), while GBDT-stump-v2 also split 5/10 against heuristic but lost 6/10 against linear-v3. We therefore keep linear-v3 as the primary learned policy in this manuscript and report tree-based guidance as exploratory negative/neutral evidence. Additional reproducibility artifacts for this comparator are:
...
For the hybrid companion objective, the current result is a bounded but positive direction: on branch-active mixed-operand workloads, pairwise-trained \texttt{hybrid\_rrn} shows repeat-matched improvements over the heuristic policy in two independent CPU-pinned confirmation passes, and remains the best-performing learned policy among the tested model families in the locked comparator pass. At the same time, the comparator evidence also shows that not all learned rankers help (the tested GBDT variant did not beat linear-v3), so we position hybrid guidance as promising but not yet final. The next required step is to extend the same protocol discipline to broader workloads and report whether this direction persists beyond policy-isolation synthetic controls.
