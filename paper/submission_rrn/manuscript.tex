...
We next evaluate the companion RRN policy path under strict policy-isolation controls. In all runs below, final consistency decisions remain symbolic; only branch ordering differs. The comparison target is the deterministic heuristic policy versus \texttt{hybrid\_rrn} with a pairwise-trained linear model (\texttt{rrn\_linear\_model\_v3\_pairwise.json}) learned from branch-snapshot traces.
