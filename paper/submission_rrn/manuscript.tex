...
This manuscript track focuses on protocol design and reproducibility boundaries for the hybrid study: identical ontology suites, timeout policy, and hardware across baseline and hybrid modes; repeated-run reporting with median/P95 wall time; and ablations that isolate guidance impact from parser and ingest effects. In branch-active synthetic policy-isolation runs, a pairwise-trained hybrid linear model shows a positive direction over heuristic ranking in two pinned-CPU confirmation passes (9/10 repeat-matched wins per pass on mixed-operand workloads). However, in an additional locked model-family comparator, linear-v3 and heuristic split repeat-matched wins (5/10 each), while a compact GBDT guidance model underperforms linear-v3. We therefore report hybrid results as workload-bounded evidence, not as a universal learned-policy advantage.
\end{abstract}
...
For hybrid guidance, prior learned-heuristic work in OWL reasoning includes ML-based heuristic selection for branch and rule ordering in tableau-style workflows \cite{mehri2019owlml}. In neighboring automated-reasoning settings, learning-guided search has been studied in connection-tableaux and broader theorem-proving pipelines \cite{farber2021connectiontableaux,blaauwbroek2024learningguided}. Neuro-symbolic RDF/DL surveys also emphasize that many neural approaches trade exactness for approximation and that robust integration with expressive DL reasoning pipelines remains limited \cite{singh2023neurosymbolic}. We therefore position RRN as a neural-symbolic policy hypothesis rather than as an entailment replacement. Recursive Reasoning Networks have been studied for ontology reasoning in neural settings \cite{hohenecker2020ontology}; in this companion track we borrow only the guidance intuition (learned relational prioritization) while keeping symbolic tableaux decisions authoritative. In this revision, \texttt{hybrid\_rrn} is an interface label for learned branch-priority policies, and the evaluated policy families are a pairwise linear scorer and a compact tree-ensemble comparator. We also include the non-neural tree baseline (gradient boosting over decision stumps \cite{friedman2001greedy}) to reduce model-choice bias in policy-level conclusions.
...
\subsection{Hybrid Branch-Prioritization Results}
\label{sec:hybrid-results}

We next evaluate the companion RRN-inspired policy path under strict policy-isolation controls. In all runs below, final consistency decisions remain symbolic; only branch ordering differs. The primary comparison target is the deterministic heuristic policy versus \texttt{hybrid\_rrn} with a pairwise-trained linear model (\texttt{rrn\_linear\_model\_v3\_pairwise.json}) learned from branch-snapshot traces.

Training and evaluation were separated operationally: model training used offline snapshot exports from distinct run IDs generated before the confirmation and comparator passes, and benchmark runs used fixed model parameters with no online updates. At the same time, training and evaluation share the same synthetic mixed-workload family by design, so the claims here should be read as in-family policy-isolation evidence rather than as out-of-distribution generalization.

Two confirmation passes were executed with identical workloads (\texttt{mixed\_operands\_8}, \texttt{mixed\_operands\_16}), repeat count (\(n=5\)), and gate settings (\(\tau=4\), \(\theta=1\mu s\)); only CPU pinning changed (Pass A: \texttt{taskset 0--11}, Pass B: \texttt{taskset 12--23}). Table~\ref{tab:hybrid-confirmation} reports median and P95 wall-clock values from the raw run CSVs.

\begin{table}[htbp]
\centering
\caption{Hybrid pairwise-policy confirmation passes (median/P95 wall-clock in ms; lower is better)}
\label{tab:hybrid-confirmation}
\resizebox{\linewidth}{!}{
\begin{tabular}{l|l|r|r|r|r}
\toprule
\textbf{Pass (CPU Set)} & \textbf{Workload} & \textbf{Heuristic Median} & \textbf{Hybrid Median} & \textbf{Heuristic P95} & \textbf{Hybrid P95} \\
\midrule
Pass A (\texttt{0--11}) & mixed\_operands\_8  & 16467.947 & \textbf{6968.183}  & 27567.529 & \textbf{26817.147} \\
Pass A (\texttt{0--11}) & mixed\_operands\_16 & \textbf{15700.232} & 15813.491 & 30040.955 & \textbf{30039.441} \\
Pass B (\texttt{12--23}) & mixed\_operands\_8  & 30102.014 & \textbf{3791.560}  & 31721.619 & \textbf{8285.933} \\
Pass B (\texttt{12--23}) & mixed\_operands\_16 & 19175.384 & \textbf{13027.817} & 30042.235 & \textbf{27718.849} \\
\bottomrule
\end{tabular}
}
\end{table}

Across repeat-matched pairs (2 workloads \(\times\) 5 repeats), hybrid won 9/10 comparisons in Pass A and 9/10 in Pass B. The model path was active in all hybrid runs (\texttt{hybrid\_model\_calls} \(>\) 0, \texttt{policy\_fallbacks} \(=\) 0). Reproducibility artifacts for this subsection are:
\texttt{spacl\_synthetic\_ablation\_20260310\_063022},
\texttt{spacl\_synthetic\_ablation\_20260310\_063405},
\texttt{spacl\_synthetic\_ablation\_20260310\_064158}, and
\texttt{spacl\_synthetic\_ablation\_20260310\_064702}
under \path{benchmarks/competitors/results/history/}.

To test model-family sensitivity, we ran a locked comparator pass with identical controls (same two workloads, same repeats, same gate overrides) across three policies: heuristic, hybrid+linear-v3, and hybrid+GBDT-stump-v2. Table~\ref{tab:hybrid-model-comparator} summarizes medians and P95 values from the raw CSV artifacts. The non-neural comparator is exploratory: it reduces bias in model-selection narrative, but it is not introduced as a replacement claim.

\begin{table}[htbp]
\centering
\caption{Hybrid model-family comparator on locked mixed-operand controls (5 repeats; lower is better)}
\label{tab:hybrid-model-comparator}
\resizebox{\linewidth}{!}{
\begin{tabular}{l|r|r|r|r}
\toprule
\textbf{Policy} & \textbf{mixed\_operands\_8 Median} & \textbf{mixed\_operands\_8 P95} & \textbf{mixed\_operands\_16 Median} & \textbf{mixed\_operands\_16 P95} \\
\midrule
Heuristic & 4064.497 & 28538.353 & 6262.586 & 30039.719 \\
Hybrid + linear-v3 (pairwise) & \textbf{2331.282} & \textbf{4461.202} & \textbf{4012.926} & \textbf{28945.686} \\
Hybrid + GBDT-stump-v2 & 10576.917 & 27794.777 & 7139.843 & 7573.800 \\
\bottomrule
\end{tabular}
}
\end{table}

In repeat-matched comparisons on this locked pass, linear-v3 and heuristic split wins (5/10 each), while GBDT-stump-v2 also split 5/10 against heuristic but lost 6/10 against linear-v3. We therefore keep linear-v3 as the primary learned-policy reference in this manuscript and report tree-based guidance as exploratory negative/neutral evidence, not as a replacement path. Additional reproducibility artifacts for this comparator are:
\texttt{spacl\_synthetic\_ablation\_20260310\_094758},
\texttt{spacl\_synthetic\_ablation\_20260310\_095023}, and
\texttt{spacl\_synthetic\_ablation\_20260310\_095728}
under \path{benchmarks/competitors/results/history/}.
...
Six limitations matter most for interpreting the results. First, the primary comparisons are deployment-level wall-clock measurements, so startup, parsing, and runtime-environment effects are part of the reported numbers. Second, the comparison set spans different logic-profile coverage, especially for ELK. Third, the current head-to-head suites are dominated by consistent ontologies, so nogood-learning gains are best read from dedicated inconsistent-workload ablations rather than from the primary benchmark tables. Fourth, the hybrid-policy evidence in Section~\ref{sec:hybrid-results} is presently limited to branch-active synthetic controls; we do not claim the same magnitude on real RDF/XML corpora yet. Fifth, runtime variance remains high in branch-heavy settings, and some runs approach practical wall-clock plateaus; therefore we report medians together with spread metrics (P95/IQR) and avoid overgeneralizing from single-run improvements. Sixth, our current learned-policy family remains narrow (linear pairwise and compact GBDT stumps), and training/evaluation overlap at workload-family level; we have not yet established stable out-of-family generalization or a deployment advantage for richer graph-neural variants under the same protocol lock.
...
For the hybrid companion objective, the current result is bounded and mixed: on branch-active mixed-operand workloads, pairwise-trained \texttt{hybrid\_rrn} shows repeat-matched improvements over the heuristic policy in two independent CPU-pinned confirmation passes, but in the locked comparator pass linear-v3 and heuristic split repeat-matched outcomes (5/10 each). The comparator evidence also shows that not all learned rankers help (the tested GBDT variant did not beat linear-v3). We therefore position hybrid guidance as a plausible policy direction with partial support, not as established superiority. The next required step is to extend the same protocol discipline to broader workloads and report whether this direction persists beyond policy-isolation synthetic controls, consistent with transferability risks emphasized in learning-guided automated-reasoning literature \cite{blaauwbroek2024learningguided,farber2021connectiontableaux}.
